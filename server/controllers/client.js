...
export const getGeography = async(req, res) => {
    try{
        ...
        // Now we need to format this information accordint to the way
        // Nivo charts wants
        // we use a reducer which starts with an empty object {} 
        // Then it adds items to the empty object as we go along
        const mappedLocations = users.reduce((acc, { country }) => {
            const countryISO3 = getCountryIso3(country); // From two letter codes to 3 letter codes
            if (!acc[countryISO3]) {
                acc[countryISO3] = 0;
            }
            acc[countryISO3] ++;
